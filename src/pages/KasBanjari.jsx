...
export default function KasBanjari() {
  ...
  const handleDownloadExcel = () => {
    try {
      ...
      XLSX.writeFile(
        wb,
        `Laporan_Kas_Al_Banjari_${new Date().getFullYear()}.xlsx`,
      );
    } catch (error) {
      console.error(error);
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text("LAPORAN KAS AL BANJARI", 105, 15, { align: "center" });
    ...
    doc.save(`Laporan_Kas_Al_Banjari.pdf`);
  };
  ...
  const recalculateSaldo = async () => {
    const q = query(collection(db, "kas_banjari"), orderBy("tanggal", "asc"));
    ...
    for (const item of snapshot.docs) {
      ...
      await updateDoc(doc(db, "kas_banjari", item.id), {
        saldo,
      });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // });
      if (isEdit) {
        await updateDoc(doc(db, "kas_banjari", editId), {
          tanggal: Timestamp.fromDate(new Date(formData.tanggal)),
          keterangan: formData.keterangan,
          masuk,
          keluar,
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      await deleteDoc(doc(db, "kas_banjari", id));
      ...
      <div className="sticky top-0 z-30 backdrop-blur-xl bg-white/70 border-b border-black/5">
        <div className="max-w-5xl mx-auto px-5 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              Keuangan Organisasi Al Banjari
            </p>
          </div>
