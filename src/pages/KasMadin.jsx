...
export default function KasMadin() {
  ...
  const handleDownloadExcel = () => {
    try {
      ...
      XLSX.writeFile(wb, `Laporan_Kas_Madin_${new Date().getFullYear()}.xlsx`);
    } catch (error) {
      console.error(error);
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text("LAPORAN KAS MADIN", 105, 15, { align: "center" });
    ...
    doc.save(`Laporan_Kas_Madin.pdf`);
  };
  ...
  const fetchKasData = async () => {
    try {
      ...
      const q = query(collection(db, "kas_madin"), orderBy("tanggal", "desc"));
  ...
  const recalculateSaldo = async () => {
    const q = query(collection(db, "kas_madin"), orderBy("tanggal", "asc"));
    ...
    for (const item of snapshot.docs) {
      ...
      await updateDoc(doc(db, "kas_madin", item.id), {
        saldo,
      });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // });
      if (isEdit) {
        await updateDoc(doc(db, "kas_madin", editId), {
          tanggal: Timestamp.fromDate(new Date(formData.tanggal)),
          keterangan: formData.keterangan,
          masuk,
          keluar,
