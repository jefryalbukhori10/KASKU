...
export default function KasPemuda() {
  ...
  const handleDownloadExcel = () => {
    try {
      ...
      const sheetData = [
        ["LAPORAN KAS PEMUDA PEMUDI RW 09"],
        [periodeText],
        [],
      ...
      XLSX.writeFile(
        wb,
        `Laporan_Kas_PEMUDA__${new Date().getFullYear()}.xlsx`,
      );
    } catch (error) {
      console.error(error);
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text("LAPORAN KAS PEMUDA PEMUDI RW 09", 105, 15, { align: "center" });
    ...
    doc.save(`Laporan_Kas_PEMUDA__${new Date().getFullYear()}.pdf`);
  };

  // const fetchKasData = async () => {
  //   const q = query(
  //     collection(db, "kas_pemuda"),
  ...
  const fetchKasData = async () => {
    try {
      ...
      const q = query(collection(db, "kas_pemuda"), orderBy("tanggal", "desc"));
  ...
  const recalculateSaldo = async () => {
    const q = query(collection(db, "kas_pemuda"), orderBy("tanggal", "asc"));
    ...
    for (const item of snapshot.docs) {
      ...
      await updateDoc(doc(db, "kas_pemuda", item.id), {
        saldo,
      });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // await addDoc(collection(db, "kas_pemuda"), {
      ...
      // });
      if (isEdit) {
        await updateDoc(doc(db, "kas_pemuda", editId), {
          tanggal: Timestamp.fromDate(new Date(formData.tanggal)),
          keterangan: formData.keterangan,
          masuk,
          keluar,
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      await deleteDoc(doc(db, "kas_pemuda", id));
  ...
  //   let message = `📊 *Laporan Kas Pemuda Pemudi RW 09*\n\n💰 Saldo saat ini: *${formatRupiah(
  //     totalSaldo
  ...
  //   message += `\n\n🔗 Lihat lebih lengkap di:\nhttps://kasku.vercel.app/kas-pemuda`;
        ...
        <div className="max-w-5xl mx-auto px-5 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-[22px] font-semibold tracking-tight text-[#111827]">
              Kas Pemuda Pemudi RW 09
            </h1>

            <p className="text-sm text-gray-500 mt-0.5">
              Keuangan Organisasi Pemuda Pemudi RW 09
            </p>
          </div>
